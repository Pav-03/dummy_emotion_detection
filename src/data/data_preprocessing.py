...
def fetch_data():
    try:
        ...
        logger.error(
            f"An unexpected error occurred while fetching data. Error detail: {e}")
        raise
...
def main():
    try:
        ...
        train_processed_data.to_csv(os.path.join(
            processed_data_path, 'train_processed.csv'), index=False)
        test_processed_data.to_csv(os.path.join(
            processed_data_path, 'test_processed.csv'), index=False)

        logger.info(
            "Processed data saved successfully in the data/processed folder.")
