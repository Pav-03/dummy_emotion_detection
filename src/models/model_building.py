    ...
    try:
        ...
        with mlflow.start_run(run_name="Xgboost-100-d3"):
            ...
            data_hash = hashlib.md5(pd.util.hash_pandas_object(
                train_data).values.tobytes()).hexdigest()

            mlflow.log_param("train_data_path",
            ...
            mlflow.log_artifact('model/vectorizer.joblib',
            ...
            logger.info(
                f"MLflow run completed successfully with run ID: {run_id}")
            ...
            logger.info(
                "MLflow run ID saved to reports/run_info.json successfully")
