...
async def auth_guard(request: Request, call_next):
    ...
    if not auth_header:
        logger.warning(
            f"No auth header for {request.method} {request.url.path}")
    ...
    if len(token) != 2 or token[0].lower() != "bearer":
        logger.warning(
            f"Invalid auth header format for {request.method} {request.url.path}")
        return JSONResponse(
            status_code=401,
    ...
    try:
        ...
        logger.info(
            f"Authenticated user_id: {request.state.user_id} for {request.method} {request.url.path}")
...
def authenticate_user(username: str, pasword: str) -> str:
    ...
    if username not in USERS:
        logger.warning(
            f"Authentication failed for username: {username} - user not found")
        raise HTTPException(
            status_code=401,
